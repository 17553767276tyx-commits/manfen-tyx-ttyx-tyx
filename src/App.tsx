...
const NoteEditor = ({ initialContent, folders, onSave, onCancel }: any) => {
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 relative z-20">
      ...
      {isEditing && (
        <div className="flex items-center gap-3 px-5 py-2 border-y border-slate-50 dark:border-slate-800/50 overflow-x-auto">
          ...
          ></button>
          <button
            onClick={() => execCmd('backColor', '#bbf7d0')}
      ...
      />
      <style>{`.editor-content:empty:before { content: attr(data-placeholder); color: #cbd5e1; } .editor-content img { max-width: 100%; border-radius: 12px; display: block; margin: 12px 0; } .read-only a { pointer-events: auto; text-decoration: underline; color: #6366f1; }`}</style>
    </div>
  );
};
...
const QuestionCard = memo(
  ({
    ...
    const isBookmarked = userState.bookmarkFolders.some((f: any) =>
    ...
    const contentSize = getFontSizeClass();
    const optionSize =
      userState.fontSize === 'small'
        ? 'text-sm'
        : userState.fontSize === 'large'
        ? 'text-lg'
        : userState.fontSize === 'xlarge'
        ? 'text-xl'
        : 'text-base';
    ...
    return (
      <div className="flex flex-col h-full relative z-10 p-4 pb-24 max-w-2xl mx-auto w-full">
        <div className="flex justify-between items-center mb-4">
          <button
            onClick={onExit}
            className="text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white flex items-center bg-white/80 dark:bg-black/40 backdrop-blur-sm p-2.5 pr-4 rounded-full shadow-sm transition-all active:scale-95"
          >
            <ChevronLeft size={20} />{' '}
        ...
        >
          ...
          {(q.type === 'fill' || q.type === 'essay') && (
            <AutoSaveTextArea
              ...
              onChange={(val: string) => onAnswerChange(q.id, val, q.type)}
            ...
            <div className="flex-1 flex justify-center px-4">
              {data.isExam && !data.examSubmitted ? (
                isLastQuestion ? (
                  ...
                  <button
                    onClick={() => onNext(0)}
                    className="flex items-center px-6 py-3 bg-slate-800 text-white dark:bg-slate-700 rounded-full font-bold shadow-lg hover:bg-slate-700 transition-all active:scale-95 w-full justify-center"
                  >
                    确认并下一题 <ChevronRight size={18} className="ml-2" />
                  </button>
                )
              ) : !data.showAnswer ? (
                <button
    ...
    return (
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center">
              <Settings className="mr-2 text-indigo-500" size={20} /> 设置
            </h3>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-screen max-w-md mx-auto overflow-hidden shadow-2xl">
        <GroupingModal /> <WrongGroupingModal /> <DeleteConfirmModal />{' '}
        <SettingsModal /> <NoteFolderModal /> <BookmarkFolderModal />{' '}
        <RandomSetupModal />
        {editingNote && (
